 ...
 */
public class AsyncDefaultLwjgl3Input implements Lwjgl3Input {
		...
		public void invoke(long window, int key, int scancode, int action, int mods) {
			switch (action) {
				case GLFW.GLFW_PRESS:
					...
					pressedKeys++;
					AsyncDefaultLwjgl3Input.this.window.getGraphics().requestRendering();
					lastCharacter = 0;
				...
				case GLFW.GLFW_RELEASE:
					pressedKeys--;
					AsyncDefaultLwjgl3Input.this.window.getGraphics().requestRendering();
					inputProcessor.keyUp(getGdxKeyCode(key));
					break;
